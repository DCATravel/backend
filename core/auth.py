...
async def get_jwks() -> Dict[str, Any]:
    ...
    if not settings.oidc_issuer_url:
        logger.error("OIDC issuer URL is not configured")
        raise ValueError("OIDC_ISSUER_URL environment variable is required")
...
def build_authorization_url(
    ...
    if not settings.oidc_issuer_url or not settings.oidc_client_id:
        logger.error("OIDC client or issuer configuration missing")
        raise ValueError("OIDC_ISSUER_URL and OIDC_CLIENT_ID must be configured")
