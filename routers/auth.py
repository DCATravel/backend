...
def get_dynamic_backend_url(request: Request) -> str:
    ...
    scheme = request.headers.get("x-forwarded-proto") or getattr(request.url, "scheme", None) or "http"
...
async def callback(
    ...
    def redirect_with_error(message: str) -> RedirectResponse:
        ...
        frontend_url = getattr(settings, "frontend_url", backend_url)
        redirect_target = f"{frontend_url}/auth/error?{fragment}" if frontend_url else f"{backend_url}/auth/error?{fragment}"
        return RedirectResponse(
            url=redirect_target,
            status_code=status.HTTP_302_FOUND,
        )
    ...
    try:
        ...
        frontend_url = getattr(settings, "frontend_url", backend_url)
        redirect_url = f"{frontend_url}/auth/callback?{fragment}" if frontend_url else f"{backend_url}/auth/callback?{fragment}"
